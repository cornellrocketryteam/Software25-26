...
- **Physical detection** (GPIO 11): Detects whether the umbilical cable is physically connected. Always active regardless of build mode.
...
Separate from the USB data link, the FSW monitors the physical umbilical connection via **GPIO 11** (input, pull-down). This is read every flight loop cycle in `FlightState::read_sensors()`.
...
1. `read_sensors()` — Reads all sensors, updates GPIO states (including umbilical sense on PIN 11)
