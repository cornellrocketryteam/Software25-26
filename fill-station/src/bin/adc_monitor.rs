...
fn main() -> Result<()> {
    ...
    let gain = Gain::One; // ±4.144V range
    ...
    loop {
        ...
        // ADC1 readings
        for i in 0..4 {
            let scaled_value = if i == 0 {
                readings[i].0 * scale_pt1
            } else if i == 1 {
                readings[i].0 * scale_pt2
            } else {
                readings[i].1
            };
            print!("{:>6} {:>6.3}V {:>8.1} │ ", readings[i].0, readings[i].1, scaled_value);
        }
