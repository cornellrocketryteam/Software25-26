...
class FillStationClient:
    ...
    def run_vent_ignite_launch_2s(self):
        """2 Sec Fart: Open SV2 + ignite, wait 2s, close SV2, wait 2s, open MAV 7.88s, close MAV."""
        def sequence():
            self.launch_status = "2s Fart: Opening SV2 + Igniting..."
            self.send_command({"command": "fsw_open_sv"})
            self.send_command({"command": "ignite"})
            time.sleep(2)
            self.launch_status = "2s Fart: Closing SV2..."
            self.send_command({"command": "fsw_close_sv"})
            time.sleep(2)
            self.launch_status = "2s Fart: MAV OPEN (7.88s)..."
    ...
    def run_vent_ignite_launch_1s(self):
        """1 Sec Fart: Open SV2 + ignite, wait 2s, close SV2, wait 1s, open MAV 7.88s, close MAV."""
        def sequence():
            self.launch_status = "1s Fart: Opening SV2 + Igniting..."
            self.send_command({"command": "fsw_open_sv"})
            self.send_command({"command": "ignite"})
            time.sleep(2)
            self.launch_status = "1s Fart: Closing SV2..."
            self.send_command({"command": "fsw_close_sv"})
            time.sleep(1)
            self.launch_status = "1s Fart: MAV OPEN (7.88s)..."
            self.send_command({"command": "fsw_open_mav"})
            time.sleep(7.88)
            self.send_command({"command": "fsw_close_mav"})
            self.launch_status = None
...
with col_mid:
    ...
    if vil_c1.button("2 SEC FART", type="primary", use_container_width=True):
        client.run_vent_ignite_launch_2s()
    if vil_c2.button("1 SEC FART", type="primary", use_container_width=True):
