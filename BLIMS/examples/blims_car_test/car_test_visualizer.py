...
if __name__ == '__main__':
    ...
    sys.exit(app.exec())
"""
visualizer.py — BLiMS real-time visualizer (pyqtgraph)
=======================================================
Reads the 13-field CSV stream from car_test.rs (or mock_gps.py /
replay_launch.py) and displays three live panels:
...
-----
    ...
    python visualizer.py /dev/pts/4     # pass port as argument
"""
...
class BlimsVisualizer(QtWidgets.QMainWindow):
    ...
    def _build_map_panel(self):
        ...
        # Manual legend labels (text items in fixed screen position)
        for text, color, y in [('● Heading', '#2196F3', 1.0),
                                ('● Bearing', '#4CAF50', 0.93),
                                ('▲ Target',  '#F44336', 0.86)]:
            ti = pg.TextItem(text, color=color, anchor=(1, 0))
    ...
    def _refresh(self):
        ...
        # Drain queue — batch all rows that arrived since last refresh
        new_rows = 0
        while True:
            try:
                row = self.row_queue.get_nowait()
        ...
        self.status.setText(
            ...
            f'Head {heading:.1f}°   Bearing {bearing:.1f}°   '
            f'Err {err:+.1f}°   Motor {motor:.3f}   '
            f'Phase: {phase_name}   Loiter: {loiter}'
        )
        ...
        # Keep both current pos and target visible
        margin = 0.002
        self.map_plot.setXRange(
            min(lon, tgt_lon) - margin, max(lon, tgt_lon) + margin, padding=0)
        self.map_plot.setYRange(
        ...
        self.p_curve.setData(rel, p_a)
        self.i_curve.setData(rel, i_a)
        self.m_curve.setData(rel, p_a + i_a)   # combined = motor deviation from 0.5
        t_now = rel[-1]
        self.pi_plot.setXRange(t_now - WINDOW_SECONDS, t_now, padding=0)
