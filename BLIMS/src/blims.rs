...
impl<'d> Blims<'d> {
    ...
    /// Direct motor command in inches; bypasses the PI controller.
    /// Used only for hardware tests (`test_blims` feature). Range [MOTOR_MIN, MOTOR_MAX].
    pub fn set_position_inches(&mut self, inches: f32) {
        self.set_brakeline_diff(inches);
    }
    ...
    fn set_brakeline_diff(&mut self, mut position: f32) {
        ...
        self.pwm_config.compare_b = duty;   // PIN_35 = PWM_SLICE9 channel B
